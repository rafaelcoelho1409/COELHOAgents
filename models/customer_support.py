...
from typing import Optional, Union, Annotated, Literal, Callable
...
class VectorStoreRetriever:
    def __init__(self, docs: list[dict], vectors: list, embeddings_model):
        ...
        Store:
         ...
         - NumPy array of their embeddings (self._arr)
         - reference to the embedding model (self._model)
        """
    ...
    def from_docs(cls, docs: list[dict], model_name: str = "qwen2.5:7b-instruct-q4_1"):
        ...
        return cls(docs, vectors, embeddings_model)
    def query(self, query: str, k: int = 5) -> list[dict]:
        """
        Compute the top-k most similar documents to the provided query.
        """
        ...
        # Return the matching docs with similarity
        return [
...
class CustomerSupport:
    def __init__(self, framework, temperature_filter, model_name, shared_memory):
        self.local_file, self.backup_file = customer_support_load_database()
        self.db = customer_support_update_dates(self.local_file, self.backup_file)
        ...
        self.retriever = VectorStoreRetriever.from_docs(self.docs)
        self.db = customer_support_update_dates(self.db, self.backup_file)
    ...
    def load_model(self):
        ...
        self.part_2_assistant_runnable = self.assistant_prompt | self.llm.bind_tools(self.part_2_tools)
        ...
        self.part_3_assistant_runnable = self.assistant_prompt | self.llm.bind_tools(
        ...
        self.builder.add_node(
            "enter_update_flight",
            self.create_entry_node("Flight Updates & Booking Assistant", "update_flight"),
        )
        ...
        self.builder.add_node(
            ...
            self.create_tool_node_with_fallback(self.update_flight_sensitive_tools),
        )
        self.builder.add_node(
            "update_flight_safe_tools",
            self.create_tool_node_with_fallback(self.update_flight_safe_tools),
        )
        ...
        self.builder.add_node(
            ...
            self.create_entry_node("Car Rental Assistant", "book_car_rental"),
        )
        ...
        self.builder.add_node(
            ...
            self.create_tool_node_with_fallback(self.book_car_rental_safe_tools),
        )
        self.builder.add_node(
            "book_car_rental_sensitive_tools",
            self.create_tool_node_with_fallback(self.book_car_rental_sensitive_tools),
        )
        ...
        self.builder.add_node(
            "enter_book_hotel", self.create_entry_node("Hotel Booking Assistant", "book_hotel")
        )
        self.builder.add_node("book_hotel", Assistant(self.book_hotel_runnable))
        self.builder.add_edge("enter_book_hotel", "book_hotel")
        self.builder.add_node(
            "book_hotel_safe_tools",
            self.create_tool_node_with_fallback(self.book_hotel_safe_tools),
        )
        self.builder.add_node(
            "book_hotel_sensitive_tools",
            self.create_tool_node_with_fallback(self.book_hotel_sensitive_tools),
        )
        ...
        self.builder.add_node(
            ...
            self.create_entry_node("Trip Recommendation Assistant", "book_excursion"),
        )
        ...
        self.builder.add_node(
            ...
            self.create_tool_node_with_fallback(self.book_excursion_safe_tools),
        )
        self.builder.add_node(
            "book_excursion_sensitive_tools",
            self.create_tool_node_with_fallback(self.book_excursion_sensitive_tools),
        )
        ...
        self.builder.add_conditional_edges(
            "book_excursion",
            self.route_book_excursion,
            ["book_excursion_safe_tools", "book_excursion_sensitive_tools", "leave_skill", END],
        )
        # Primary assistant
        self.builder.add_node("primary_assistant", Assistant(self.assistant_runnable))
        self.builder.add_node(
            "primary_assistant_tools", self.create_tool_node_with_fallback(self.primary_assistant_tools)
        )
        ...
        #self.builder.add_node("assistant", Assistant(self.part_1_assistant_runnable))
        #self.builder.add_node("safe_tools", self.create_tool_node_with_fallback(
        #    #self.part_1_tools
        #    #self.part_2_tools
        #    self.part_3_safe_tools
        #    ))
        #self.builder.add_node(
        #    "sensitive_tools", self.create_tool_node_with_fallback(
        #        self.part_3_sensitive_tools)
        #)
        #self.builder.add_edge("fetch_user_info", "assistant")
        ...
        ##)
        #self.builder.add_conditional_edges(
        #    "assistant", self.route_tools, ["safe_tools", "sensitive_tools", END]
        #)
        #self.builder.add_edge("safe_tools", "assistant")
        #self.builder.add_edge("sensitive_tools", "assistant")
    ...
    def create_all_assistants(self):
        ...
        self.update_flight_sensitive_tools = [self.update_ticket_to_new_flight, self.cancel_ticket]
        self.update_flight_tools = self.update_flight_safe_tools + self.update_flight_sensitive_tools
        self.update_flight_runnable = self.flight_booking_prompt | self.llm.bind_tools(
        ...
        self.book_hotel_sensitive_tools = [self.book_hotel, self.update_hotel, self.cancel_hotel]
        self.book_hotel_tools = self.book_hotel_safe_tools + self.book_hotel_sensitive_tools
        self.book_hotel_runnable = self.book_hotel_prompt | self.llm.bind_tools(
        ...
        self.book_car_rental_runnable = self.book_car_rental_prompt | self.llm.bind_tools(
        ...
        self.book_excursion_sensitive_tools = [self.book_excursion, self.update_excursion, self.cancel_excursion]
        self.book_excursion_tools = self.book_excursion_safe_tools + self.book_excursion_sensitive_tools
        self.book_excursion_runnable = self.book_excursion_prompt | self.llm.bind_tools(
    ...
    #---FLIGHT TOOLS---#
    @tool
    def fetch_user_flight_information(self, config: RunnableConfig) -> list[dict]:
    ...
    @tool
    def cancel_ticket(self, ticket_no: str, *, config: RunnableConfig) -> str:
...
###FUNCTIONS
@st.cache_resource
def customer_support_load_database():
    with st.spinner("Loading database"):
        ...
        return local_file, backup_file


# Convert the flights to present time for our tutorial
@st.cache_resource
def customer_support_update_dates(local_file, backup_file):
